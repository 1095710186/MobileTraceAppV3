...
public class TXJHD_Activity extends BaseActivity {
    @BindView(R.id.pTV)
    TextView mPTV;
    @BindView(R.id.cTV)
    TextView mCTV;
        ...
        mAdapter.setOnListItemClick(new OnImageListItemClick() {
            ...
            public void OnClickItem(View view, int position, boolean isAdd) {
                if (position == foodInfoList.size()) {
                   ...
                   FoodInfo foodInfo = mAdapter.getItemData(position);//菜品列表   id
                    if (CommonUtil.isNetWorkConnected(mActivity)) {
                        getVendorData(foodInfo); //查询对应的产地列表
                    } else {
      ...
      /*  recyclerView.addOnItemTouchListener(new OnItemClickListener() {
            ...
            public void SimpleOnItemClick(BaseQuickAdapter baseQuickAdapter, View view, int i) {
                ...
                if (i == mAdapter.getCount()){
                    ...
                    FoodInfo foodInfo = mAdapter.getItem(i);//菜品列表   id
                    *//*for (FoodInfo info : foodInfoList) { // 已入场得菜品  itemId
                        if (info.getImgId().equals(foodInfo.getImgId())) {
                            info.setId(foodInfo.getId());
                            foodInfo = info;
                            break;
                        }
                    }*//*
                        ...
                        if ( wrapper.getFoodInfos().size() > 0) {
                            ...
                            for (FoodInfo itemIdF:wrapper.getFoodInfos()){
                                for (FoodInfo idF:foodInfoList) {
                                    if (idF.getId().equals(isEdit?itemIdF.getItemId():itemIdF.getId())) {
...
//        PhotoUtil.open(this, true, 1);
        url_gp_path=  PhotoUtil.takePhoto(TXJHD_Activity.this);
